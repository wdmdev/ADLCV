...
def sample_stratified(
    ...
    distances = torch.linspace(near, far, n_samples, device=rays_o.device) # HINT: USE torch.linspace, get device with rays_o.device 
...
def prepare_viewdirs_chunks(points, rays_d, encoding_function, chunksize=2**15, constant_viewdir=None):
    ...
    if constant_viewdir is not None:
        viewdirs = torch.ones_like(rays_d) * constant_viewdir # TASK 4: Render with a constant viewdir. HINT: Use torch.ones_line(...) and constant_viewdir
    else :
